...
import { formatBRL } from '../storage/apacStorage';
...
export default function ApacBalancetePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setErro('');
      try {
        const [finRaw, despRaw] = await Promise.all([
          financeiroApi.loadFinanceiro(),
          despesasApi.loadDespesas(),
        ]);
        setFinanceiro(financeiroApi.mapFinanceiroUi(finRaw));
        setDespesas(despesasApi.mapDespesasUi(despRaw));
      } catch (err) {
        setErro(
          err instanceof ApiError
            ? err.message
            : 'Não foi possível carregar dados do balancete. Verifique a API.'
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <PageShell
      title="Balancete"
      subtitle="Relatório financeiro consolidado (Financeiro + Despesas no banco)"
    >
      ...
      {loading ? (
        ...
        <>
          ...
          <p className="text-muted small mt-4 mb-0">
            Total de saídas inclui lançamentos do Financeiro e do módulo Despesas.
          </p>
        </>
      )}
    </PageShell>
